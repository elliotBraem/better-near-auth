...
import { createDatabaseDriver, type DatabaseDriver } from "./db/driver";
...
export default createPlugin({
  ...
  secrets: z.object({
    ...
    AUTH_DATABASE_DRIVER: z.enum(["libsql", "better-sqlite3", "bun", "node"]).optional(),
      ...
      const driver = yield* Effect.acquireRelease(
        Effect.sync(() =>
          createDatabaseDriver(config.secrets.AUTH_DATABASE_URL, {
            driver: config.secrets.AUTH_DATABASE_DRIVER,
  ...
  createRouter: (services, builder) => {
    ...
    return {
      ...
      listApiKeys: builder.listApiKeys.use(requireAuth).handler(async ({ context, input }) => {
        ...
        return (result as { apiKeys?: Array<{ id: string; name: string; prefix: string; createdAt: Date }> }).apiKeys ?? [];
      }),
      ...
      createApiKey: builder.createApiKey.use(requireAuth).handler(async ({ input, context }) => {
        const result = await safeAuthApi(() =>
          (services.auth.api.createApiKey as (args: {
            headers: Headers;
            body: unknown;
          }) => Promise<{ id: string; name: string; prefix: string; key: string; createdAt: Date }>)({
            headers: createHeaders(context.reqHeaders),
            body: input,
          }),
        );
      ...
      nearVerify: builder.nearVerify.handler(async ({ input }) => {
        ...
        return result as { token: string; success: true; user: { id: string; accountId: string; network: "mainnet" | "testnet" } };
      }),
      ...
      nearProfile: builder.nearProfile.use(requireAuth).handler(async ({ input, context }) => {
        ...
        return result as { name?: string; description?: string; image?: { url?: string; ipfs_cid?: string }; backgroundImage?: { url?: string; ipfs_cid?: string }; linktree?: Record<string, string> } | null;
      }),

      nearLinkAccount: builder.nearLinkAccount.use(requireAuth).handler(async ({ input, context }) => {
        const result = await safeAuthApi(() =>
        ...
        return result as { success: boolean; accountId: string; network: string; message: string };
      }),

      nearUnlinkAccount: builder.nearUnlinkAccount.use(requireAuth).handler(async ({ input, context }) => {
        const result = await safeAuthApi(() =>
      ...
      nearListAccounts: builder.nearListAccounts.use(requireAuth).handler(async ({ context }) => {
        ...
        return result as { accounts: Array<{ id: string; userId: string; accountId: string; network: string; publicKey: string; isPrimary: boolean; createdAt: Date }> };
      }),
      ...
      nearRelayStatus: builder.nearRelayStatus.use(requireAuth).handler(async ({ input, context }) => {
        const result = await safeAuthApi(() =>
        ...
        return result as { status: "pending" | "completed" | "failed"; gasUsed?: string; outcome?: unknown };
      }),
      ...
      nearRelayerInfo: builder.nearRelayerInfo.use(requireAuth).handler(async ({ context }) => {
        ...
        return result as { enabled: boolean; accountId?: string; mode?: "ephemeral" | "explicit"; network?: "mainnet" | "testnet"; balance?: string; available?: string; staked?: string; storageUsage?: string; storageBytes?: number; hasContract?: boolean; hasKey?: boolean; createdAt?: string; lastUsedAt?: string };
      }),
      ...
      nearRelayHistory: builder.nearRelayHistory.use(requireAuth).handler(async ({ context }) => {
        ...
        return result as { transactions: Array<{ id: string; userId: string; txHash: string; senderId: string; receiverId: string; network: string; status: string; gasUsed?: string; createdAt: string; updatedAt?: string }> };
      }),
